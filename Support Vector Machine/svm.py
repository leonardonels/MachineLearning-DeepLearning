...
class SVM:
    ...
    def loss(self, y_true, y_pred):
        ...
        y_pred: np.array
            predicted labels in [-inf, inf]. shape=(n_examples,)
        ...
        # return np.sum(np.maximum(0,1-y_true*y_pred))/y_true.shape[0]
        return (self._lambda/2)*np.linalg.norm(self._w)**2+np.sum(np.maximum(0,1-y_true*y_pred))/y_true.shape[0]
    ...
    def predict(self, X):
        ...
        return np.digitize(np.dot(X,self._w), bins=[0])
